...
const load_kanji_data = () => {
    ...
    .then(data => {
        console.log(data[prob_counter]);
        prob_num_data = data[prob_counter];
...
const updateExplanationContent = async(event) => {
    ...
    var answer_value = document.getElementById('input_answer')
    var user_answer = answer_value.value
    answer_form.reset();
    prob_num_data = await load_kanji_data(); //非同期処理
...
const check_answer = (prob_num_data, user_answer) =>{
    ...
    result = [];
    var kunyomi_ja = prob_num_data['kunyomi_ja']; //"やわ、やわらぐ、やわらげる、なご、なごむ、なごやか"
    var onyomi_ja_katakana = prob_num_data['onyomi_ja']; //"ワ、オ"
    var onyomi_ja_hiragana = prob_num_data['onyomi_hiragana']; //"わ、お"

    var kunyomi_roma = prob_num_data['kunyomi_roma'];
    var onyomi_roma = prob_num_data['onyomi_roma']

    var divided_string_list_ja = kunyomi_ja.split("、").concat(onyomi_ja_katakana.split("、")).concat(onyomi_ja_hiragana.split("、"));
    var answer_data_ja = divided_string_list_ja.filter(item => (item != '' && item != 'n/a'));
    var divided_string_list_roma = kunyomi_roma.split(",").concat(onyomi_roma.split(","));
    var nospace_divided_string_list_roma = divided_string_list_roma.map(function(item){
        return item.replace(/\s+/g, '');
    });
    var answer_data_roma = nospace_divided_string_list_roma.filter(item => (item != '' && item != 'n/a'));
    var answer_data = answer_data_ja.concat(answer_data_roma);
    console.log('answer_data = ' + answer_data);
    console.log('user_answer = ' + user_answer);
    if(answer_data.includes(user_answer)){
